#include <jni.h>
#include <string>

extern "C" JNIEXPORT jstring JNICALL
Java_com_electrodragon_YasirFirstProject_1admin_provider_ElectroLibAccessProvider_getMasterKey(JNIEnv* env, jobject _j) {
    std::string s = "";
    return env->NewStringUTF(s.c_str());
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_electrodragon_YasirFirstProject_1admin_provider_ElectroLibAccessProvider_getSecretKey(JNIEnv* env, jobject _j) {
    std::string s = "9QTBV2PL3AM378TJM994A8IFHV0Q9H8PFQA9DBNJEC1T9SU3UNAGY0Z3R7D3X8AV8YQJCK";
    return env->NewStringUTF(s.c_str());
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_electrodragon_YasirFirstProject_1admin_provider_ElectroLibAccessProvider_getServerBaseUrl(JNIEnv* env, jobject _j) {
    std::string s = "https://api.YasirFirstProject.com/";
    return env->NewStringUTF(s.c_str());
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_electrodragon_YasirFirstProject_1admin_provider_ElectroLibAccessProvider_getServerApiKey(JNIEnv* env, jobject _j) {
    std::string s = "hi,-its-eevee. I can do wonderful things in api creation.";
    return env->NewStringUTF(s.c_str());
}
